var AppSession = "../Compras/RegistroFacturaCompraMilagros.aspx";
...
$(document).ready(function () {
    ...
    $('#MainContent_txtDescuento').blur(function () {
        try {
            if ($('#MainContent_txtDescuento').val() == '')
                $('#MainContent_txtDescuento').val('0.00');

            F_RecalcularDescuento($('#hfImporte').val(), $('#MainContent_txtDescuento').val());
        }
        catch (e) {
    ...
    forceNumber($('#MainContent_txtImporte'));

    forceNumber($('#MainContent_txtDescuento'));
...
function F_CamposAlternativas(Campos) {
    switch (Campos) {
        case "MainContent_txtFechaIngreso":
            $('#MainContent_ddlFormaPago').focus();
            break;
        case "MainContent_txtProveedor":
            if ($('#hfCodCtaCte').val() != "0")
                $('#MainContent_ddlMoneda').focus();
            else
            { alertify.log('ESPECIFIQUE PROVEEDOR'); }
            break;
        case "MainContent_ddlIgv":
            $('#MainContent_txtNumeroGuia').focus();
            break;
        case "MainContent_txtNumeroGuia":
            $('#MainContent_ddlAlmacenFisico').focus();
            break;
        case "MainContent_ddlAlmacenFisico":
            $('#MainContent_ddlTipoDocumento2').focus();
            break;
        case "MainContent_ddlTipoDocumento2":
            $('#MainContent_btnAgregarProducto').click();
            break;
        case "MainContent_chkConIgvMaestro":
            $('#MainContent_chkSinIgvMaestro').focus();
            break;
        case "MainContent_chkSinIgvMaestro":
            $('#MainContent_btnAgregarProducto').click();
            break;
...
function F_TablaUp() {
    ...
    try {
        ...
        if ($(ctrlPosActual.replace(ant, pos)).length > 0) {
            ...
            $(ctrlPosActual).closest("tr").children('td,th').css("background-color", "#FFF8D1")
        }
...
function F_ValidarImporteGrilla(ControlID) {
    if ($("#" + ControlID).val() == '')
        $("#" + ControlID).val('0.00')
...
function F_ElegirEmpresa(Fila) {
    ...
    $('#hfCodAlmacen').val($(hfCodEmpresa_Grilla).val());
    $('#divSeleccionarEmpresa').dialog('close');
    MostrarEspera(false);
    $('#MainContent_txtEmision').focus();
}
...
function F_ElegirEmpresa2() {
    ...
    $('#MainContent_hdnCodEmpresa').val($('#hfCodEmpresa').val());
    $('#hfCodSede').val($('#hfCodEmpresa').val());
    $('#MainContent_hdnCodSede').val($('#hfCodEmpresa').val());
    $('#divSeleccionarEmpresa').dialog('close');
}
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        $('#MainContent_ddlTipoDocumento2').css('background', '#FFFFE0');
                        $('#MainContent_ddlTipoDocumento3').css('background', '#FFFFE0');
        ...
        var objParams =
            {
                ...
                Filtro_CodAlmacenFisico: 0 //$('#MainContent_ddlAlmacenFisico').val()
            };
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        ctrlPosActual = '#MainContent_grvConsultaArticulo_chkOK_0';
                        $(ctrlPosActual).closest("tr").children('td,th').css("background-color", "#FFF8D1")
                        $(ctrlPosActual).focus();
...
function F_RecalcularDescuento(Importe, Descuento) {
    try {
        var mImporte = Number(Importe);
        var mDescuento = Number(Descuento);
        var mIGV = parseFloat($("#MainContent_ddlIgv option:selected").text());

        var mSubtotal = (mImporte - mDescuento);
        mIGV = (mSubtotal * mIGV);
        var mTotal = parseFloat(mSubtotal) + parseFloat(mIGV);

        // SUBTOTAL BRUTO-DESCUENTO
        $('#MainContent_txtImporte').val(mImporte.toFixed(2));
        $('#MainContent_txtDescuento').val(mDescuento.toFixed(2));
        $('#MainContent_txtSubTotal').val(mSubtotal.toFixed(2));
        $('#MainContent_txtIgv').val(mIGV.toFixed(2));
        $('#MainContent_txtTotal').val(mTotal.toFixed(2));
        $('#MainContent_txtMonto').val(mTotal.toFixed(2));
    }
    catch (e) {
...
function F_GrabarDocumento() {
    ...
    try {
        ...
        var objParams = {
            ...
            Filtro_CodAlmacen: $('#hfCodAlmacen').val(),
            Filtro_CodTipoDocSust: $(Contenedor + 'ddlTipoDocumento2').val(),
            ...
            Filtro_ImporteBruto: $(Contenedor + 'txtImporte').val(),
            Filtro_TipoCambio: $(Contenedor + 'lblTC').text(),
            ...
            Filtro_CodTipoDoc: $(Contenedor + 'ddlTipoDocumento2').val(),
...
function F_Nuevo() {
    ...
    $('#MainContent_txtImporte').val('0.00');
    $('#hfImporte').val('0.00');
    $('#MainContent_txtDescuento').val('0.00');
...
function F_Buscar() {
    try {
        ...
        var objParams = {
            ...
            Filtro_CodTipoDocSust: $('#MainContent_ddlTipoDocumento3').val(),
...
function F_EditarRegistro(Fila) {
    ...
    try {
        ...
        $('#hfCodDocumentoVenta').val($(hfCodigo).val());
...
function F_ContarRegistros() {
    ...
    $('#MainContent_grvDetalleArticulo .chkDelete :checkbox').each(function () {
        chkSi = '#' + this.id;
        hfCodDetalle = chkSi.replace('chkEliminar', 'hfCodDetalle');
        lblNumId = chkSi.replace('chkEliminar', 'lblNumId');
        if ($(hfCodDetalle).val() != '') {
            Count++;
            $(lblNumId).text(Count);
        }
...
function F_ImprimirFacturaPDF(Fila) {
    ...
    var CodMenu = '217';
    var hfCodigo = $("#" + Fila.id.replace("imgPdf", "hfCodigo")).text();
...
function F_ReemplazarDocumento(Fila) {
    try {
        ...
        var lblFlagIgv = '#' + imgID.replace('imgReemplazar', 'hfFlagIgv');
        var lblTasaIgv = '#' + imgID.replace('imgReemplazar', 'hfTasaIgv');
        ...
        if ($(lblFlagIgv).val() == '1') {
            tasaigv = parseFloat($(lblTasaIgv).val()) + parseFloat(1);
            FlagIgv = 1;
        }
        ...
        F_ReemplazarFactura_NET(arg, function (result) {
            ...
            if (str_resultado_operacion == "1") {
                ...
                $('#MainContent_ddlTipoDocumento2').val($(lblTipoDoc).val());
                ...
                $('#MainContent_txtImporte').val(result.split('~')[6]);
                ...
                $('#MainContent_txtFechaIngreso').val(result.split('~')[28]);
                ...
                $('#MainContent_ddlAlmacenFisico').val(result.split('~')[27]);
                ...
                $('#MainContent_txtNumeroGuia').val(result.split('~')[26]);
