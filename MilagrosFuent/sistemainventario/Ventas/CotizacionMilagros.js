...
//            $('#MainContent_ddlFormaPago').val(i.item.FormaPago);
            ...
            $('#hfCodTransportista').val(i.item.CodTransportista);
               $('#MainContent_txtCelularCliente').val(i.item.Celular);
...
//$('#MainContent_txtCliente').keydown(function (e) {
...
//            val: "3489",  
//            NroRuc: "11111111",
//            label: "11111111 - CLIENTE VARIOS",
//            Direccion: "INDEPENDENCIA",
//            DireccionEnvio: "LIMA LIMA INDEPENDENCIA",
//            Distrito: "LIMA LIMA INDEPENDENCIA",
//            CodDepartamento: "1",
//            CodProvincia: "8",
//            CodDistrito: "16",
//            CodTransportista: "0",
//            Transportista: "",
//            Comentario: "CLIENTE VARIOS",
//            CodRuta: "1",
//            FlagIncluyeIgv: "1"
//        };

//        
//        $('#hfCodCtaCte').val(item.val);
//        $('#MainContent_txtNroRuc').val(item.NroRuc);
//        $('#MainContent_txtCliente').val(item.label);
//        $('#MainContent_txtDireccion').val(item.Direccion);
//        $('#MainContent_txtDestino').val(item.DireccionEnvio);
//        $('#MainContent_txtDistrito').val(item.Distrito);
//        $('#hfCodDepartamento').val(item.CodDepartamento);
//        $('#hfCodProvincia').val(item.CodProvincia);
//        $('#hfCodDistrito').val(item.CodDistrito);
//        $('#hfCodTransportista').val(item.CodTransportista);
//        $('#MainContent_txtTransportista').val(item.Transportista);
//        $('#MainContent_txtObservacionCliente').val(item.Comentario);
//        $('#MainContent_ddlRuta').val(item.CodRuta);
...
//             }
              ...
              if ((CantidadArticulos-parseFloat($('#MainContent_lblNumRegistros').text()))<0) {
             if (F_PermisoOpcion(CodigoMenu, 777600, 'Insertar') == "0") {            
            toastr.warning('Debe retirar : ' +  parseFloat($('#MainContent_lblcantarticulos').text())*-1 + ' articulo');
            return false;}
             }
            ...
            (
                ...
                function (result) {
                    ...
                    if (str_resultado_operacion == "1") {
                        ...
                        F_Update_Division_HTML('div_Ruta', result.split('~')[18]);
                        ...
                        $('#MainContent_txtCelularCliente').css('background', '#FFFFE0'); 
                        $("#MainContent_txtCelularCliente").ForceNumericOnly();        
...
function F_ValidarGrabarDocumento() {
    try {
        ...
        if ($(Cuerpo + 'txtTotal').val() == '0.00')
          ...
          var celular = $(Cuerpo + 'txtCelularCliente').val().trim();
...
function F_GrabarDocumento(vistaprevia) {
    try {
        ...
        var objParams = {
            ...
            Filtro_Celular:$('#MainContent_txtCelularCliente').val()
        };
...
function F_Nuevo(Codigo, CodProforma) {
    ...
    $('#MainContent_txtCliente').val('');
    $('#MainContent_txtPlaca').val('');
   $('#MainContent_txtCelularCliente').val('');
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Anular') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacio
    try {
        ...
        var lblFactura = '#' + imgID.replace('imgAnularDocumento', 'lblFactura');
        ...
        if ($(lblFactura).text() !="") {
            toastr.warning("ESTA COTIZACION SE ENCUENTRA CANCELADA, PRIMERO ELIMINE LA COBRANZA Y LUEGO ANULE LA COTIZACION");
            return false;
        }
...
function ImprimirFacturaDetalle(Fila) {
    ...
    rptURL = rptURL + 'Impresora=' + Impresora + '&';
    rptURL += 'NombreTabla2=' + encodeURIComponent('MiTabla') + '&';
    rptURL += 'NombreArchivo2=' + encodeURIComponent('MiArchivo') + '&';
            ...
            $.ajax({
                ...
                if (data.length > 0)
               {
                    ...
                    $('#txtSaldoCreditoFavor').text(data[0].split(',')[12]);
                          $('#MainContent_txtCelularCliente').val(data[0].split(',')[16]);
...
if (F_PermisoOpcion(CodigoMenu, CodigoInterno, 'Editar') === "0") return false; //Entra a /Scripts/Utilitarios.js.F_PermisosOpcion para mas informacio
    try {
        ...
        var lblFactura = imgID.replace('imgReemplazar', 'lblFactura');

        if ($(lblEstado).text() == "ANULADO") {
            toastr.warning(""+ $("#MainContent_ddlTipoDocConsulta option:selected").text()   + " SE ENCUENTRA ANULADA");
            return false;
        }

        if ($(lblEstado).text() !="PENDIENTE") {
            toastr.warning(" "+ $("#MainContent_ddlTipoDocConsulta option:selected").text()   + " TIENE PROFORMA O FACTURA");
            return false;
        }

        if ($(lblFactura).text() !="") {
            toastr.warning(" "+ $("#MainContent_ddlTipoDocConsulta option:selected").text()   + " TIENE BOLETA O FACTURA");
            return false;
        }
...
function F_AprobarCotizacion(Fila) {
 ...
 try 
        {
    var imgID = Fila.id;
    var lblCodMarcaGv = '#' + imgID.replace('imgAprobar', 'hfCodigo');
    var lblEstado = '#' + imgID.replace('imgAprobar', 'lblEstado');
    var lblNumero = '#' + imgID.replace('imgAprobar', 'lblNumero');
    var lblCliente = '#' + imgID.replace('imgAprobar', 'lblcliente');
    var hfTipoDocumento = '#' + imgID.replace('imgAprobar', 'hfTipoDocumento');    
    if ($(lblEstado).text()=='ANULADO')
    {
        toastr.warning("LA COTIZACION SE ENCUENTRA ANULADA");
        return false;
    ...
    {
        toastr.warning("LA COTIZACION YA SE ENCUENTRA APROBADO");
        return false;
    }
    if(!confirm("ESTA SEGURO DE CONFIRMAR LA COTIZACION "  + " : " + $(lblNumero).text() + "\nDE : " +  $(lblCliente).text()))
    return false;
