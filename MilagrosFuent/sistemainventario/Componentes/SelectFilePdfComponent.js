...
class SelectFilePdfComponent extends HTMLElement {
    ...
    _currentFileFilter = ['pdf']; //filtro de archivos tipo jpg png pdf etc etc
  ...
  constructor( { divContainer, Id, Base64, heightInPX, getBase64, title, smallButtons = false }) {
    ...
    this._segmentToRestore = '               <embed id="' + this._previewHolder + '" style="width: 100%; height: ' + this._heightImage + '; object-fit: scale-down;" type="application/pdf"> ';
    ...
    '         <div id="' + this._div_imgContainer + '" class="SelectFileComponentPDF" style="width: 100%; height: ' + this._heightContainer + ';"> ' +
    '               <embed id="' + this._previewHolder + '" style="width: 100%; height: ' + this._heightImage + '; object-fit: scale-down;" type="application/pdf"> '  +
    '         </div> ' +
    '     </div> ' +
    ...
    clearImg() {
        ...
        $('#' + this._div_imgContainer).append(_segmentToRestore);
        return;
    }
    ...
    restoreImg() {
        if ((this._originalBase64 == null || this._originalBase64 == undefined) || this._originalBase64 == "" ) {
            ...
            $('#' + this._div_imgContainer).append(this._segmentToRestore);
            $("#" + this._iImage1).val(null);
            $("#" + this._iImage2).val("");
        } else {
            this.cargarView(this._originalBase64)
    ...
    restoreToPrevious() {
        if ((this._lastBase64 == null || this._lastBase64 == undefined) || this._lastBase64 == "" ) {
            ...
            this.cargarView(this._lastBase64.base64)
    ...
    readURL(input) {
       ...
       if (input.files && input.files[0]) {
        ...
        if (allowedExtensions.indexOf(extension) === -1) {
            ...
            alert('El archivo no es el formato permitido');
            return;
        }

        var reader = new FileReader();
        reader.onload = function(e) {
            _this.cargarView({ base64: e.target.result } )
    ...
    cargarView = async (archivo) => {
        const pdfstr = await fetch(archivo.base64);   //pdf is the base64 string
        ...
        $('#' + this._previewHolder).attr('src', blobUrl);
        return;       
    }
...
customElements.define("select-file-pdf-component", SelectFilePdfComponent);